...
class Model(object):
    ...
    def update_pi(self, R):
        ...
        # Compute summations
        sum_iN_rk = R.sum(axis=0) # Vector for each k
        sum_kK_iN_rk = sum_iN_rk.sum()

        return (sum_iN_rk + A - 1) / \
               (sum_kK_iN_rk + (K * A) - K)
    ...
    def update_theta(self, X, R):
        ...
        # Output tensor should be K x D (dimensions of theta)
        # X is N x D
        # R is N x K-> R_T is K x N
        # Thus, R_T(K x N) * X(N x D) -> W(K x D)
        # W[row, col] = sum(i=1->N) (r_row^(i) * x_col^(i))
        # Sum over all samples of (row's responsibility * column's attribute)
        R_T = R.transpose()
        sum_iN_rki_xji = R_T @ X
        sum_iN_rki_1_minus_xji = R_T @ (1 - X)

        first = sum_iN_rki_xji + (a - 1)
        second = (sum_iN_rki_1_minus_xji) + (b - 1)
    ...
    def compute_posterior(self, X, M=None):
        ...
        ######################## Your code here #########################
        X_masked = X * M
        log_pi_k = np.log(self.params.pi)
        log_theta_kj = np.log(self.params.theta)
        log_1_minus_theta_kj = np.log(1 - self.params.theta)

        log_p_z_x = log_pi_k + \
                    (X_masked @ log_theta_kj.transpose()) + \
                    ((1 - X_masked) @ log_1_minus_theta_kj.transpose())
    ...
    def posterior_predictive_means(self, X, M):
        ...
        return posterior @ self.params.theta
